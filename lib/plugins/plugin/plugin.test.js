...
describe('Plugin', () => {
  ...
  describe('#constructor()', () => {
    ...
    it('should run promise chain in order for "plugin:searhc:search" hook', () => {
  ...
  describe('#install()', () => {
    ...
    beforeEach(() => {
      ...
      getPluginsStub = sinon
        .stub(plugin, 'getPlugins').returns(BbPromise.resolve(plugins));
      execSyncStub = sinon
        .stub(childProcess, 'execSync').returns();
      tmpDirPath = testUtils.getTmpDirPath();
      serverlessYmlFilePath = path.join(tmpDirPath, 'serverless.yml');
      packageJsonFilePath = path.join(tmpDirPath, 'package.json');
      plugin.serverless.config.servicePath = tmpDirPath;
    });
    ...
    it('should not install the plugin if it can not be found in the registry', () => {
      ...
      return plugin.install().then(() => {
        ...
        const packageJsonContent = JSON.parse(fs.readFileSync(packageJsonFilePath).toString());
        expect(packageJsonContent.dependencies).to.deep.equal({});
    ...
    it('should log a message if a problem during installation happens', () => {
      ...
      const packageJsonFileContent = {
        name: 'my-serverless-service',
        dependencies: {}, // plugin was not added successfully
      };
      fse.writeJsonSync(packageJsonFilePath, packageJsonFileContent);
    ...
    it('should generate a package.json file in the servide directory if not present', () => {
      // serverless.yml
      const serverlessYml = {
        service: 'plugin-service',
        provider: 'aws',
    ...
    it('should add the plugin to the service file if plugins array is not present', () => {
      ...
      return plugin.install().then(() => {
        ...
        // inspect the package.json file
        const packageJsonContent = JSON.parse(fs.readFileSync(packageJsonFilePath).toString());
        expect(packageJsonContent.dependencies).to.deep.equal(packageJsonFileContent.dependencies);
    ...
    it('should push the plugin to the service file if plugins array is present', () => {
      // serverless.yml
      const serverlessYml = {
        service: 'plugin-service',
        provider: 'aws',
    ...
    it('should push the plugin to the service file if an empty plugins array is present', () => {
      // serverless.yml
      const serverlessYml = {
        service: 'plugin-service',
        provider: 'aws',
  ...
  describe('#uninstall()', () => {
    ...
    it('should only remove the given plugin from the service', () => {
      ...
      const packageJsonFileContent = {
        name: 'my-serverless-service',
        dependencies: {}, // plugin was removed via npm
      };
      fse.writeJsonSync(packageJsonFilePath, packageJsonFileContent);
